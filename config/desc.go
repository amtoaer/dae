...
4. "domain++". Based on domain+ mode but force to re-route traffic using sniffed domain to partially recover domain based traffic split ability. It doesn't work for direct traffic and consumes more CPU resources.`,
	...
	"subscription_user_agent":      "Custom User-Agent for fetching subscriptions. If not set, dae uses its default User-Agent. Per-subscription User-Agent can be set via URL fragment, for example https://example.com/sub#ua=V2rayA.",
